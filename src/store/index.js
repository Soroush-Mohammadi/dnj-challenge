...
export default new Vuex.Store({
  state: {
    user: {
      name: "Admin",
      avatar:
        "https://www.pinpng.com/pngs/m/127-1275400_paul-circle-testimonial-male-face-icon-png-transparent.png",
    },
    discussions: [
      {
        id: 3,
  ...
  mutations: {
    ...
    addComment(state, payload) {
      state.discussions.find((item) => {
        if (item.id == payload.id) {
          item.replies.push(payload);
        }
      });
    },
    addReplyLikes(state, payload) {
      state.discussions.forEach((element) => {
        element.replies.find((item) => {
          if (item.id == payload) {
            item.likes++;
          }
        });
    ...
    addNewDiscussion(state, payload) {
      state.discussions.push(payload);
    },
  },
  actions: {
    async addDiscussion({ commit }, payload) {
      await commit("addNewDiscussion", payload);
    },
  },
  modules: {},
});
